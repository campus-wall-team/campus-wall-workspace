#!/usr/bin/env bash
# 本地运行 graphrag（chat=本机 Ollama qwen2.5:7b，embedding=本机 Ollama bge-m3，
# Neo4j=已有 campus-neo4j 容器）。离线、零成本、不碰云端 key。
#
# 用法：
#   cp demo/.env.example demo/.env     # 首次：填入真实 NEO4J_PASSWORD
#   bash demo/run-graphrag.sh          # 启动服务（前台），监听 :8001
set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
GRAPHRAG_DIR="$(cd "$SCRIPT_DIR/../campus-wall-graphrag" && pwd)"
...
export GRAPHRAG_PORT="${GRAPHRAG_PORT:-8001}"
export VECTOR_INDEX_NAME="${VECTOR_INDEX_NAME:-campus_chunk_vector}"

cd "$GRAPHRAG_DIR"
exec .venv/bin/uvicorn app.main:app --host 0.0.0.0 --port "$GRAPHRAG_PORT"
