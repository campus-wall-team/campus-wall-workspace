# Campus Wall — GraphRAG「AI 学长」本地 Demo
...
```
小程序 → 后端 POST /api/v1/ai-senior/chat → GraphRAG :8001 /query
...
| **Python venv** | GraphRAG 服务依赖 | `campus-wall-graphrag/.venv`（见该项目 README） |
...
| 文件 | 作用 |
|------|------|
| `run-graphrag.sh` | 启动脚本：加载 `demo/.env` → 清理 socks 代理 → 设 Neo4j / Chat / Embed 环境变量 → `uvicorn app.main:app --port 8001` |
...
- **接全栈**：后端 `application-dev.yaml` 的 `GRAPHRAG_BASE_URL` 默认指向 `http://localhost:8001`，启动 Spring Boot 后即可通过 `/api/v1/ai-senior/chat` 走完整链路。
